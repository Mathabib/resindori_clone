 ...
 * @since		1.5
 */
class modMainMenuNoixHelper
{
	function buildXML(&$params)
	{
		$menu = new JMenuNoixTree($params);
		$items = &JSite::getMenu();
	...
	function &getXML($type, &$params, $decorator)
	{
		...
		if (!isset($xmls[$type])) {
			$cache =& JFactory::getCache('mod_mainmenu_noix');
			$string = $cache->call(array('modMainMenuNoixHelper', 'buildXML'), $params);
			$xmls[$type] = $string;
		}
 ...
 * @since		1.5
 */
class JMenuNoixTree extends JTree
{
	/**
	...
	{
		$this->_params		=& $params;
		$this->_root		= new JMenuNoixNode(0, 'ROOT');
 ...
 * @since		1.5
 */
class JMenuNoixNode extends JNode
{
	/**
	 * Node Title
	 */
